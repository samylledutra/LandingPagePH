...
function HeroSection({ onButtonClick }) {
  ...
  useEffect(() => {
    ...
    setBackgroundImage(isMobile ? BannerMobile : Banner);
  }, []);

  return (
    <section className="firstSection relative pb-[22rem] px-[32px] bg-cover bg-right" style={{ backgroundImage: `url(${backgroundImage})`}}>
      ...
      <div className="relative z-10 max-w-[65rem] mx-auto px-4 mt-[-5rem]">
        <h1 className="uppercase pt-[70px] pr-[480px] text-[26px] font-bold leading-tight text-customBlue mb-6 ">
        Você percebe que há algo além do que os olhos conseguem enxergar? 
        </h1>
        <p className="mt-4 pr-[500px] text-[18px] text-justify">
